...
<!--<div class="add_place none" id="autorized">-->
    <div class="place_form" style="width:617px;height:650px;">
        
      <h3>Edit Business<span></span></h3>
      <% ViewPlaceBean user=PlaceManager.getbusinessuserbyemail((String) session.getAttribute("user"));  %>
      
      <form action="EditBusinessServlet" method="post">
          
        <label>Name: <input type="text" name="name" value="<%=user.getPlace_name() %>"></label>
       
        <label>Address:   <input type="text" disabled="true" name="address" value=<%= user.getAddress() %>></label>
        <label>Website:   <input type="text" name="web" value=<%= user.getWebsite_name() %>> </label>
        <label>Contact Number:<input type="text" name="contact" value=<%= user.getContact_no() %>></label>
        <label>Description:<textarea name="desc" style="width:100%" cols="20" rows="5"><%= user.getDescription() %></textarea></label>
